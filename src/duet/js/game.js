...
async function playerColors(player, element) {
  ...
  if (player === 1) {
    colorChange = new ColorChange(
      solution.returnSolution().player1Solution,
      solution.returnList()
    );
  } else {
    colorChange = new ColorChange(
      solution.returnSolution().player2Solution,
      solution.returnList()
    );
  }
...
show1Element.addEventListener('click', async function () {
  ...
  console.log('test');
  console.log(solution.returnSolution()); // missing on backup
...
hideElement.addEventListener('click', async function () {
  document.getElementById('solutionGrid').innerHTML = '';
});

document.getElementById(
  'solution_button'
).href = `../solution/index.html?code=${getParam('code')}`;
